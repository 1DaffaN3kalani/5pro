...
cl.login(token="EnJ3d96UfaoGOjWlweQ6.d0Y8Z+kWn1tqFy24ChaJ9G.BTzgxvaw1BiQq8v1J8STDu+QjFd8Dn4LyFf+pi+5V40=")
cl.loginResult()

ki = LINETCR.LINE() #One
ki.login(token="Eno0IlHPgZ6tLsXOx1s1.fs2cgHLX6TOYCA89m8LcSq.7WRd4MJ1cFKJTIZWv6DAA0l2DvnlY/5o9w8AoNO8X+s=")
ki.loginResult()

kk = LINETCR.LINE() #Two
kk.login(token="EnsRNWOMc5xJyZVZugI7.sFdOlGKavE/JPdmWVLjKzW.jU59rsx/uz2FB0G7HDQCMWzHGsyL3wUryycJnWLh8w8=")
kk.loginResult()

kc = LINETCR.LINE() #Tree
kc.login(token="EntnQiTe39CsmVFfhsx3.PkKNsTykUChFu4RWKDTN0W.unJFKV5P4468VDmXTU/Q8EyA/jl5srzF7cYmUIbAYzA=")
kc.loginResult()

ks = LINETCR.LINE() #Four
ks.login(token="En35TOa6rEATNdauwVb9.8ccvTR8aHloLYcm86rZIEq.xPZDU9crPRdu/GRumwXMOapfVnngEKvfZgGpa8/6zNc=")
ks.loginResult()

ka = LINETCR.LINE() #Five
ka.login(token="EntxHlW38HK0uCXpwYd2.IWEgVNlTR9u6NogMK/G0SG.ULSM6/NtyHWn69oN6ZJq91iYi4ngl474sZiNlOk8LVo=")
ka.loginResult()
          ...
          if wait["Protectgr"] == True:
            if cl.getGroup(op.param1).preventJoinByTicket == False:
              if op.param2 in Bots:
                ...
                try:
                  ki.sendText(op.param1,cl.getContact(op.param2).displayName + "Wooyyy Please Dont Playing Link Group BROO!!!")
                  ...
                  if targets == []:
                    ...
                    for target in targets:
                      try:
                        ...
                        ks.sendText(msg.to,"Already Invited Done✔💋: \n➡" + _name)
                        wait["winvite5"] = False
                        break
                      except:
                        try:
...
#-----------------------------------------------
    ...
    #-------------Fungsi Respon Finish---------------------#   
              ...
              if msg.from_ in owner:
                wait["winvite2"] = True
                ki.sendText(msg.to,"Share contact Bebs 😘😘")
                
            elif msg.text in ["3invite"]:
              if msg.from_ in owner:
                wait["winvite3"] = True
                kk.sendText(msg.to,"Share contact Bebs 😘😘")
                
            elif msg.text in ["4invite"]:
              if msg.from_ in owner:
                wait["winvite4"] = True
                kc.sendText(msg.to,"Share contact Bebs 😘😘")
                
            elif msg.text in ["5invite"]:
              if msg.from_ in owner:
                wait["winvite5"] = True
                ks.sendText(msg.to,"Share contact Bebs 😘😘")
                
            elif msg.text in ["6invite"]:
              if msg.from_ in owner:
                wait["winvite6"] = True
                ka.sendText(msg.to,"Share contact Bebs 😘😘")
