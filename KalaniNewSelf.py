...
ka.login(token="EoTgd7nANPmF7Rv7qyv1.cnCKeS8mJyEU5JSZppFleq.cZFbzld/wrX1+yNVQpvyQODYEOClEzu+6jdVzYzIbeA=")
ka.loginResult()
...
admin=["u0f4cbccff2b03754d07d8db8707023f6","u152d821a1971eada3271af3d33c942b1"] 
        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != {'mid': mid}:
                 if wait["detect"] == True:
                    ...
                    balas = ["Apaan sich tag mulu naksir ya.? ",cName + " Ngapain ngetag???",cName + " Gak sah tag.Kalau ada perlu pm aja",cName + " Dasar diLarang keras sebut² nama gue Tag mulu Pm aja klo pentingヽ(^0^)ﾉ"]
                    ret_ = "[Auto Respond] \n\n" + random.choice(balas)
...
#---------------------
        ...
        if op.type == 15:
          if op.param2 in Bots:
            return
          random.choice(KAC).sendText(op.param1, "[ Bye Bye 🙋🙋 Broo Sampai jumpa kembali ]")
